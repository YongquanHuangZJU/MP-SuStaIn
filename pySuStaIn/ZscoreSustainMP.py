###
# MP-SuStaIn: Python implementation of the Multi-Parametric Subtype and Stage Inference (MP-SuStaIn) algorithm, the code is modified from pySuStaIn
# The following comments are the original license and citation information for pySuStaIn.
###
...
#*******************************************
#The data structure class for ZscoreSustainMP is the same are the ZscoreSustain. It holds the z-scored data that gets passed around and re-indexed in places.
...
class ZscoreSustainMP(AbstractSustain):
    ...
    def __init__(self,
        ...
        ### === MP-SuStaIn added Parameters ===
        #   multi_labels                - in the MP-SuStaIn, the biomarkers are considered as either "independent" or "correlated to one another", the "multi_labels" is a vector specifying the multi parametric biomarkers. The label is 0 for independent biomarkers, and 1,2,... for correlated biomarkers. For example, for biomarkers [a,b,c,d,e,f], if a,b are independent, c,d are correlated, e,f are correlated, then multi_labels = [0,0,1,2,1,2]
        #   corr_coeff                  - the numbrt of correlation coefficient between correlated biomarkers 
        ### ===================================
        ...
        ### === MP-SuStaIn added parts ===
        self.multi_feature_num          = int(np.max(multi_labels)) # The number of the biomarkers in the correlated biomarkers group
        self.multi_feature_group_num       = int(len(multi_labels[multi_labels>0])/self.multi_feature_num) # The number of the correlated biomarkers group
        multi_labels_index              = np.where(multi_labels>0)[0]
        self.sorted_multi_labels_index  = multi_labels_index[np.argsort(multi_labels[multi_labels_index])] # sort the index to calculate
        
        self.corr_coeff                  = corr_coeff / (1-corr_coeff*corr_coeff) # for bivariate gaussian function, the exponent part is  -1/(2(1-rho^2))*(x^2+y^2 - 2rho xy), so here we calculate rho/(1-rho^2) once to save time in likelihood calculation
        corr_coeff_labels                = (np.array(multi_labels>0)).astype("float32")
        corr_coeff_labels                = corr_coeff_labels / (1-corr_coeff*corr_coeff) 
        corr_coeff_labels[np.where(corr_coeff_labels == 0)] = 1 # for correlated biomarkers, the coefficient is 1/(1-rho^2), for independent biomarkers, the coefficient is 1
        self.corr_coeff_labels           = np.tile(corr_coeff_labels[None, :, None], (data.shape[0], 1, numStages+1))
        
        # sigmat and factor are previously in function _calculate_likelihood_stage, since the calculation is same in model fitting, here move it to init to save time
        self.sigmat                      = np.tile(np.array(self.std_biomarker_zscore)[np.newaxis, :, np.newaxis], (data.shape[0], 1, stage_biomarker_index.shape[1]+1))
        self.factor                      = np.tile(np.log(1. / np.sqrt(np.pi * 2.0) * np.array(self.std_biomarker_zscore))[np.newaxis, :, np.newaxis], (data.shape[0], 1, stage_biomarker_index.shape[1]+1))
        ### === ============================
    ...
    def _calculate_likelihood_stage(self, sustainData, S):
        ...
        possible_biomarkers                 = range(self.data.shape[1])
        ...
        for i in range(B):
            ...
            for j in range(len(event_location) - 1):
                ...
                if j == 0:  # FIXME: nasty hack to get Matlab indexing to match up - necessary here because indices are used for linspace limits
                    ...
                    point_value[i, temp]    = ZscoreSustainMP.linspace_local2(event_value[j], event_value[j + 1], N_j, arange_N[0:N_j])
                else:
                    # original
        ...
        x = (sustainData.data[:, :, None] - stage_value) / self.sigmat[:sustainData.data.shape[0], :, :]
        ### === MP-SuStaIn modified parts ===
        # extract the multi parametric biomarkers using index
        x_multi = x[:, self.sorted_multi_labels_index, :].reshape(-1, self.multi_feature_num, self.multi_feature_group_num, x.shape[2])
        p_perm_k = coeff + np.sum(self.factor[:x.shape[0], :, :] - 0.5 * self.corr_coeff_labels[:x.shape[0], :, :] * np.square(x), 1) +  np.sum(self.corr_coeff * np.prod(x_multi, axis=1), 1) # execute bivariate gaussian log likelihood for correlated biomarkers and normal gaussian log likelihood for independent biomarkers
        ### === =============================
        p_perm_k = np.exp(p_perm_k)
    ...
    def _plot_sustain_model(self, *args, **kwargs):
        return ZscoreSustainMP.plot_positional_var(*args, Z_vals=self.Z_vals, **kwargs)
    ...
    def subtype_and_stage_individuals_newData(self, data_new, samples_sequence, samples_f, N_samples):
        ...
        ### === MP-SuStaIn modified parts ===
        sigmat_temp                     = self.sigmat # temporally keep the original sigmat
        factor_temp                     = self.factor # temporally keep the original factor
        corr_coeff_labels_temp          = self.corr_coeff_labels # temporally keep the original corr_coeff_labels

        self.sigmat                     = np.tile(np.array(self.std_biomarker_zscore)[np.newaxis, :, np.newaxis], (data_new.shape[0], 1, self.stage_biomarker_index.shape[1]+1)) # modified sigmat to new data shape
        self.factor                     = np.tile(np.log(1. / np.sqrt(np.pi * 2.0) * np.array(self.std_biomarker_zscore))[np.newaxis, :, np.newaxis], (data_new.shape[0], 1, self.stage_biomarker_index.shape[1]+1)) # modified factor to new data shape
        self.corr_coeff_labels          = np.tile(corr_coeff_labels_temp[0, :, :], (data_new.shape[0], 1, 1)) # modified corr_coeff_labels to new data shape
        ...
        # restore the original sigmat, factor and corr_coeff_labels
        self.corr_coeff_labels          = corr_coeff_labels_temp
        self.sigmat                     = sigmat_temp
    ...
    def plot_positional_var(samples_sequence, samples_f, n_samples, Z_vals, biomarker_labels=None, ml_f_EM=None, cval=False, subtype_order=None, biomarker_order=None, title_font_size=12, stage_font_size=10, stage_label='SuStaIn Stage', stage_rot=0, stage_interval=1, label_font_size=10, label_rot=0, cmap="original", biomarker_colours=None, figsize=None, subtype_titles=None, separate_subtypes=False, save_path=None, save_kwargs={}):
        ...
        stage_zscore_int = stage_zscore.astype('int8')
        zvalues = np.unique(stage_zscore_int)
        ...
        if cmap == "original":
            ...
            colour_mat = np.array([[1, 0, 0], [1, 0, 1], [0, 0, 1], [0, 0.2, 1], [0, 0.4, 1], [0, 0.6, 1], [0, 0.8, 1], [0, 1, 1]])[:N_z]
        ...
        figs = []
        this_samples_sequence = samples_sequence[subtype_order[0],:,:].T
        N = this_samples_sequence.shape[1]
        matrix = np.empty(shape=(N_bio,N,3,0)) #TODO, add a matrix
        ...
        for i in range(subtype_loops):
            ...
            for j in range(total_axes):
                ...
                if i not in range(N_S):
                    ax.set_axis_off()
                    continue
                this_samples_sequence = samples_sequence[subtype_order[i],:,:].T #(100000, N_stage)
                N = this_samples_sequence.shape[1]
                ...
                # print("stage_zscore: ", stage_zscore) #TODO, (1, N_stage) vector
                confus_matrix = (this_samples_sequence==np.arange(N)[:, None, None]).sum(1) / this_samples_sequence.shape[0] #TODO, (N_stage, N_stage)
                # print("confus: ", confus_matrix[:, 0])
                # Define the confusion matrix to insert the colours
                # Use 1s to start with all white
                confus_matrix_c = np.ones((N_bio, N, 3))

                # stage_zscore_int = stage_zscore_int.astype('int8')
                # Loop over each z-score event
                for j, z in enumerate(zvalues): #TODO, j uses for the index, z means the Z-score
                    ...
                    alter_level = colour_mat[j] == 0 #TODO alter: 用于减去其他channel，保留设定的channel颜色
                    # Extract the uncertainties for this z-score
                    # print(confus_matrix.shape)
                    # print(stage_zscore.shape)
                    # print((stage_zscore_int==z)[0])
                    # print(confus_matrix[(stage_zscore_int==z)[0]].shape)
                    # print(stage_zscore[(stage_zscore_int==z)].shape)
                    confus_matrix_zscore = np.multiply((stage_zscore[(stage_zscore_int==z)]/(z+(z==0)))[:,None], confus_matrix[(stage_zscore_int==z)[0]]) #TODO (Z_score number, N_stage)
                    # array_test = np.ones((confus_matrix_zscore.shape[0], 1))
                    # confus_matrix_zscore = np.multiply(array_test, confus_matrix[(stage_zscore_int==z)[0]]) #TODO (Z_score number, N_stage)
                    # confus_matrix_zscore = confus_matrix[(stage_zscore_int==z)[0]] #TODO (Z_score number, N_stage)
                    
                    # Subtract the certainty for this colour
                    confus_matrix_c[
                        np.ix_(
                            stage_biomarker_index[(stage_zscore_int==z)[0]], range(N),
                            alter_level
                        ) #定位三维矩阵的下标
                    ]-= np.tile(
                        confus_matrix_zscore.reshape((stage_zscore_int==z).sum(), N, 1),
                        (1, 1, alter_level.sum())
                    ) #TODO, confus_matrix_c的shape: N_bio * N_stage * RGB, 用于画图
                if subtype_titles is not None:
                ...
                matrix = np.concatenate([matrix, confus_matrix_c[:, :, :, np.newaxis]], axis=3) #TODO, add the confus matrix
        ...
        return figs, axs, matrix   #TODO, return matrix
